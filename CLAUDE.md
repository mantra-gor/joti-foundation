...
- A form's schema entry is `true` / `false` (required or not), or an object for the cases needing more: `oneOf` (the partnership and volunteer selects) and `min` + `minMessage`, which move one form's length floor off the shared rule's — `/volunteer`'s optional "area of interest" box takes 2 characters where a `/contact` message still takes 10. Override `min` only with a `minMessage` alongside it; the rule's own message names the rule's own number.
...
- Client-side validation is a UX layer, **not** a security boundary. The Server Actions revalidate with `validateForm` from the same module (see `careers/actions.js` and `partner-with-us/actions.js` — trim, validate against the shared schema, return a generic `error`). The two legacy Route Handlers (`api/contact`, `api/volunteer`) still only check for falsiness and should make the same move.
...
- `/` — Home. Built: all 7 sections from the shared design (hero + stats strip, purpose/vision/mission, core values grid, operational excellence, impact stats, closing CTA), plus a partner ribbon between impact stats and the closing CTA that isn't in the Figma frame — added on request. It's driven by `src/lib/partners.js` (same pattern as `reports.js`: add an entry, drop the logo in `public/images/clients/`, no component change) and is the one place the "no ambient looping animation" rule is deliberately broken — it pauses on hover and falls back to a static wrapped row under `prefers-reduced-motion`. The ribbon is **logo-only**: every entry needs a `logo`, and the text-wordmark fallback for logo-less partners was removed on 2026-08-07 along with the placeholder entries (NDRF, SEEDS, Start Network, Indian Institute of Hydrology) that used it. Supplied artwork is **not** a consistent shape (a padded 250x200 canvas for most, a tight-cropped 4.7:1 wordmark for Zomato), so each entry carries its own intrinsic `width`/`height` and the component normalises display size with a height plus a `max-w-*` cap. Read a new logo's real pixel dimensions off the file — guessing letterboxes its slot in the row. Photography is still styled-div placeholders with intended `alt` text left in code comments — swap for real images when available, and eyedrop the exact Emergency Amber hex from Figma (`--color-accent` in `globals.css` is currently a placeholder).
...
- `/volunteer` — Built: registration form (name, email, phone, city, country, work preference, message), posting via `postJson("/api/volunteer", …)` to the local Route Handler (see "Forms & integrations"). Only name/email/phone are required — it's a register-your-interest form and every mandatory field costs sign-ups; the optional ones are labelled `(optional)` because `Input`/`Select` have no visual required marker. The field set, the `Remote work / On-site work / Hybrid` preference options, and the validation schema live in `./volunteerFields.js`, imported by the form *and* by `src/app/api/volunteer/route.js` (same arrangement as `careers/careersFields.js`, just reaching across into `api/` because volunteer is one of the two legacy Route Handlers). `city` / `country` / `preference` are not mailbox columns — they land in the row's `payload`, so no mailbox-side change was needed. No Figma design exists yet for this page.
- `/partner-with-us` — Built: partnership enquiry form (name, organisation, email, phone, partnership type, message) submitting via the `submitPartnershipEnquiry` Server Action in `./actions.js`. The partnership-type select and its validation schema both live in `./partnershipTypes.js`, imported by the form *and* the action so the client's `oneOf` guard and the server-side allowlist can't drift. Its eight options are deliberately **broader** than the four illustrative partner cards on `/work-with-us#partnership` (they mirrored each other 1:1 until 2026-08-14) — the cards are a curated overview, this is the full intake taxonomy, so don't "resync" them. Selecting `Other (please specify)` reveals a required free-text `partnershipDetail` field; the action folds it into the recorded type as `Other: <detail>` and ignores it for every other selection. No Figma design exists yet.
- `/careers` — Built: application form (name, email, phone, city, country, role, CV/portfolio **link**, "fit & purpose" message) submitting via the `submitCareersApplication` Server Action in `./actions.js`. **Every field is required** as of 2026-08-14 — phone and the link used to be optional. The field set and its validation schema live in `./careersFields.js`, imported by the form *and* the action so live validation and the server-side check can't drift (same arrangement as `partner-with-us/partnershipTypes.js`). `city` / `country` are not mailbox columns; they land in the row's `payload` like `position` and `portfolio`, so adding them needed no mailbox-side change — but they aren't filterable in Filament either. A link rather than a file upload on purpose: the mailbox ingest API is JSON-only, so there is nowhere to put a file. No Figma design exists yet.
